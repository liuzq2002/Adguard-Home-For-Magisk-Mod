...
if [ -d "$AGH_DIR" ]; then
  ...
  if [ "$key_click" = "1" ]; then
    ...
    mkdir -p "$BACKUP_DIR"
    [ -f "$BIN_DIR/AdGuardHome.yaml" ] && cp -f "$BIN_DIR/AdGuardHome.yaml" "$BACKUP_DIR/" 2>/dev/null
    [ -f "$SCRIPT_DIR/config.sh" ] && cp -f "$SCRIPT_DIR/config.sh" "$BACKUP_DIR/" 2>/dev/null
    [ -f "$SCRIPT_DIR/NoAdsService.sh" ] && cp -f "$SCRIPT_DIR/NoAdsService.sh" "$BACKUP_DIR/" 2>/dev/null
